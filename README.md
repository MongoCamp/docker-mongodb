...
```bash
docker pull mongocamp/mongodb:8.3.10
```
...
```bash
docker run --publish 27017:27017 mongocamp/mongodb:8.3.10
```
...
| MONGO_REPLICA_SKIP_INIT        | false                | If `true`, skip automatic `rs.initiate()` during startup even when `MONGO_REPLICA_SET_NAME` is set. Useful for adding secondary nodes to the cluster.                                                                                   |
